...
Add `--sandbox sbx` to run the worker in Docker Sandboxes instead of on the host:

```bash
herdr agent start reviewer --kind codex --pane "$review_pane" --sandbox sbx -- -m gpt-5.4
```

This uses the pane's current shell directory as the SBX workspace and fails closed: the CLI sends the distinct `agent.start_sbx` API method and never retries with host `agent.start`. Claude, Codex, and Kiro are supported initially. Herdr waits for positive idle-screen evidence from the inner agent, so SBX image or microVM setup output is not treated as ready.

For a thin-host leader, configure the leader's own agent harness to expose only the Herdr layout and agent commands needed to create panes, call sandboxed start, prompt workers, inspect output, and wait. Herdr does not remove shell or filesystem tools from the leader, and its same-user socket is not a capability boundary; those leader restrictions must come from the harness. SBX enforces worker containment.
...
To keep the helper off the host, use the same recipe with `--sandbox sbx` on the `agent start` command.
