...
Herdr can launch supported agents through [Docker Sandboxes](https://docs.docker.com/ai/sandboxes/) while keeping its normal naming, prompting, reading, and waiting workflow:

```bash
herdr agent start developer --kind codex --pane w1:p2 --sandbox sbx
```

The pane's live shell directory becomes the SBX workspace. Herdr runs a uniquely named `sbx run` sandbox and never falls back to the host executable if the SBX launch method is unavailable. Managed SBX startup currently supports Claude, Codex, and Kiro because their screen manifests provide positive evidence that the inner agent is ready; other kinds fail with `unsupported_sbx_agent_kind`.

Herdr recognizes those exact `sbx run` wrapper commands on Linux, macOS, and Windows. Recognition proves which agent UI Herdr should inspect, not that a process is contained. Docker SBX owns the microVM, filesystem, credential, and network boundary.
