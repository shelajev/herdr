...
`--sandbox sbx` launches through Docker Sandboxes in the pane shell's current directory. It uses the separate `agent.start_sbx` API method and never falls back to starting the agent executable on the host. SBX launch currently accepts `claude`, `codex`, and `kiro`; unsupported kinds return `unsupported_sbx_agent_kind`. Herdr derives a stable, session-scoped sandbox name from the agent name and passes arguments after `--` only to the inner agent.
